...
async function callGemini(messages: OpenAiMessage[], options: GenerateWithFallbackOptions, model: string): Promise<string> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) throw new Error('GEMINI_API_KEY is missing.');

  const systemInstruction = options.systemInstruction || messages.find((m) => m.role === 'system')?.content;
...
async function callGrok(messages: OpenAiMessage[], options: GenerateWithFallbackOptions, model: string): Promise<string> {
  ...
  try {
    const response = await fetch('https://api.x.ai/v1/chat/completions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` },
      body: JSON.stringify({ model, messages: fallbackMessages, temperature: options.temperature ?? 0.2, max_tokens: options.maxOutputTokens ?? 1400, stream: false }),
      signal: controller.signal
    });
    if (!response.ok) {
      const detail = await response.text().catch(() => '');
...
export async function generateWithFallback(options: GenerateWithFallbackOptions): Promise<NormalizedAiResponse> {
  ...
  if (process.env.GEMINI_API_KEY) {
    try {
      const text = await callGemini(messages, options, geminiModel);
      if (text) return { text, provider: 'gemini', modelUsed: geminiModel, fallbackTriggered: true, fallbackReason: primaryError?.message || 'OpenAI service unavailable' };
  ...
  if (process.env.XAI_API_KEY) {
    try {
      ...
      if (text) return { text, provider: 'grok', modelUsed: grokModel, fallbackTriggered: true, fallbackReason };
    } catch (err: any) {
