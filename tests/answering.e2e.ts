...
async function run() {
  ...
  // ---- Grounding instructions actually reach the model ----
  {
    ...
    check('the model is told not to pad a single-fact answer with unneeded structure', /a single-fact lookup needs nothing beyond the lead sentence/i.test(system));
  }
