/**
 * Re-fetches the app's typefaces from Google and rewrites public/fonts as
 * self-hosted files.
 *
 * Run this to update a typeface, not on every build — the point of vendoring
 * them is that the app never reaches for a CDN at runtime.
 *
...
const FAMILIES = [
  {
    family: 'Inter',
    slug: 'inter',
    source: 'https://fonts.googleapis.com/css2?family=Inter:wght@400;450;500;600;700&display=swap',
    note: 'the interface sans'
  },
  {
    // Newsreader is the Chambers serif. Its optical-size axis has to be pinned
    // in the request, or Google returns a static instance per size step rather
    // than the one variable file that covers the range.
    family: 'Newsreader',
    slug: 'newsreader',
    source: 'https://fonts.googleapis.com/css2?family=Newsreader:opsz,wght@6..72,400..700&display=swap',
    note: 'the Chambers serif — headings, document titles, editorial copy'
  }
];

mkdirSync(OUT, { recursive: true });

for (const { family, slug, source, note } of FAMILIES) {
  const css = await (await fetch(source, { headers: { 'User-Agent': UA } })).text();
  const blocks = [...css.matchAll(/\/\* ([a-z-]+) \*\/\s*(@font-face \{.*?\})/gs)];
  if (blocks.length === 0) throw new Error(`no @font-face blocks came back for ${family}`);

  const faces = [];
  for (const subset of SUBSETS) {
    const block = blocks.find(([, name]) => name === subset)?.[2];
    if (!block) throw new Error(`subset ${subset} missing from upstream css for ${family}`);

    const url = block.match(/url\((https:\/\/[^)]+\.woff2)\)/)?.[1];
    const range = block.match(/unicode-range: ([^;]+);/)?.[1];
    if (!url || !range) throw new Error(`could not parse the ${subset} face for ${family}`);

    const file = `${slug}-var-${subset}.woff2`;
    ...
    console.log(`${file.padEnd(32)} ${(bytes.length / 1024).toFixed(0)} KB`);
  ...
  /* One variable file covers the whole range. Google serves the same file for
     ...
     same file repeatedly and pinned a variable face to a single weight. */
  font-weight: 400 700;
  font-display: swap;
  ...
  writeFileSync(
    new URL(`${slug}.css`, OUT),
    `/* Self-hosted ${family} (${note}), latin + latin-ext.
