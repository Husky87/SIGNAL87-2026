...
export async function generateWithFallback(
  ...
  if (process.env.GEMINI_API_KEY) {
    ...
    // 2. Retry with secondary Gemini model (gemini-3.1-flash-lite) if 503/429/error
    const secondaryModel = geminiModel === 'gemini-3.1-flash-lite' ? 'gemini-3.6-flash' : 'gemini-3.1-flash-lite';
    try {
      // Small 600ms backoff
