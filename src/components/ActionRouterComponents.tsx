...
export const GeminiMarkdownRenderer: React.FC<{ text: string; citations?: Citation[]; onSelectDocument?: (doc: any) => void; documents?: any[]; }> = ({ text, citations, onSelectDocument, documents }) => {
  ...
  return <div className="s87-prose text-[16px] leading-[1.7] text-[var(--ink)] tracking-normal">{blocks.map((block, idx) => {
    ...
    if (block.type === 'list') return <ul key={idx} className="list-disc pl-6 my-3 space-y-1.5">{(block.items || []).map((item, itemIdx) => <li key={itemIdx} className="pl-1">{parseInlineStyles(item, citations, onSelectDocument, documents)}</li>)}</ul>;
    return <p key={idx} className="my-0 mb-4 last:mb-0 text-[var(--ink)]">{parseInlineStyles(block.content || '', citations, onSelectDocument, documents)}</p>;
  })}</div>;
};
