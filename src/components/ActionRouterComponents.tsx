...
export const parseInlineStyles = (text: string, citations?: Citation[], onSelectDocument?: (doc: any) => void, documents?: any[]) => {
  const withMathSymbols = text.replace(/\$\\(rightarrow|leftarrow|times|leq|geq|pm|approx|neq|cdot|div)\$/g, (_match, cmd: string) => {
    const symbols: Record<string, string> = { rightarrow: '→', leftarrow: '←', times: '×', leq: '≤', geq: '≥', pm: '±', approx: '≈', neq: '≠', cdot: '·', div: '÷' };
    return symbols[cmd] || _match;
  });
  const clean = withMathSymbols.replace(/^#+\s*/, '');
  const parts = clean.split(/(`[^`]+`|\*\*[^*]+\*\*|\*[^*]+\*|\[\d+(?:\s*,\s*\d+)*\]|\[CIT-\d+\]|\[SPA-\d+\.\d+\])/g);
  return parts.map((part, idx) => {
    if (part.startsWith('`') && part.endsWith('`')) return <code key={idx} className="bg-[var(--surface-2)] text-[var(--ink)] border border-[var(--rule)] px-1.5 py-0.5 rounded-[3px] text-[11px] font-mono font-bold tracking-tight inline-block mx-0.5" style={{ fontFamily: 'var(--mono)' }}>{part.slice(1, -1)}</code>;
    if (part.startsWith('**') && part.endsWith('**')) return <strong key={idx} className="font-semibold text-[var(--ink)]">{part.slice(2, -2)}</strong>;
    if (part.startsWith('*') && part.endsWith('*')) return <em key={idx} className="italic text-[var(--ink-2)]">{part.slice(1, -1)}</em>;
    if (part.startsWith('[') && part.endsWith(']')) {
      const label = part.slice(1, -1); let indices: number[] = [];
      if (/^\d+(\s*,\s*\d+)*$/.test(label)) indices = label.split(',').map((n) => parseInt(n.trim(), 10) - 1);
      else if (label.startsWith('CIT-')) indices = [parseInt(label.replace('CIT-', ''), 10) - 1];
      else if (label.startsWith('SPA-')) { if (label.includes('8.2')) indices = [0]; else if (label.includes('8.4')) indices = [1]; else if (label.includes('8.7')) indices = [2]; }
      const validIndices = citations && citations.length > 0 ? indices.filter((n) => n >= 0 && n < citations.length) : [];
      if (validIndices.length === 0) return null;
      return <React.Fragment key={idx}>{validIndices.map((citationIndex) => { const cite = citations![citationIndex]; return <button key={citationIndex} onClick={() => { if (onSelectDocument) { const matched = documents?.find((d) => d.id === cite.docId || d.title.toLowerCase().includes(cite.docTitle.toLowerCase()) || cite.docTitle.toLowerCase().includes(d.title.toLowerCase())); if (matched) onSelectDocument(matched); else onSelectDocument({ id: cite.docId || `doc-${Date.now()}`, title: cite.docTitle || 'Document', type: 'PDF', sizeBytes: 1024 * 1024 * 2.4, uploadDate: new Date().toLocaleDateString(), tags: ['Citation', 'Verified'], owner: 'Signal87 AI', organization: 'Signal87 Enterprise', status: 'Ready', aiIndexed: true, embeddingsComplete: true, versionHistory: [], permissions: 'Project Only', summary: cite.snippet || 'Grounded citation reference for this synthesis.', category: 'Legal' }); } }} className="text-[var(--teal)] hover:opacity-80 font-bold text-xs cursor-pointer align-super mx-0.5 select-none hover:underline" title={cite.paragraphRef ? `View: ${cite.docTitle} (${cite.paragraphRef})` : `View: ${cite.docTitle}`}>[{citationIndex + 1}]</button>; })}</React.Fragment>;
    }
    return part;
  });
};

export const GeminiMarkdownRenderer: React.FC<{ text: string; citations?: Citation[]; onSelectDocument?: (doc: any) => void; documents?: any[]; }> = ({ text, citations, onSelectDocument, documents }) => {
  ...
  return <div className="text-[15.5px] sm:text-[16px] leading-[1.6] text-[var(--ink)] tracking-normal space-y-1">{blocks.map((block, idx) => {
    ...
    if (block.type === 'heading') { const cleanText = parseInlineStyles(block.content || '', citations, onSelectDocument, documents); if (block.level === 1) return <h1 key={idx} className="font-sans text-xl sm:text-2xl font-bold text-[var(--ink)] mt-6 mb-2.5 tracking-tight">{cleanText}</h1>; if (block.level === 2) return <h2 key={idx} className="font-sans text-lg sm:text-xl font-semibold text-[var(--ink)] mt-5 mb-2 tracking-tight">{cleanText}</h2>; return <h3 key={idx} className="font-sans text-base sm:text-lg font-semibold text-[var(--ink)] mt-4 mb-1.5">{cleanText}</h3>; }
    if (block.type === 'table') return <div key={idx} className="my-4 overflow-x-auto border border-[var(--rule)] rounded-[4px]"><table className="w-full text-sm border-collapse"><thead><tr className="bg-[var(--surface-2)]">{(block.tableHeaders || []).map((header, hIdx) => <th key={hIdx} className="text-left px-3 py-2 border-b border-[var(--rule)] font-semibold text-[var(--ink)] whitespace-nowrap">{parseInlineStyles(header, citations, onSelectDocument, documents)}</th>)}</tr></thead><tbody>{(block.tableRows || []).map((row, rIdx) => <tr key={rIdx} className="border-b border-[var(--rule)] last:border-b-0">{row.map((cell, cIdx) => <td key={cIdx} className="px-3 py-2 align-top text-[var(--ink-2)]">{parseInlineStyles(cell, citations, onSelectDocument, documents)}</td>)}</tr>)}</tbody></table></div>;
    if (block.type === 'list') return <ul key={idx} className="list-disc pl-5 my-2 space-y-1">{(block.items || []).map((item, itemIdx) => <li key={itemIdx} className="pl-1">{parseInlineStyles(item, citations, onSelectDocument, documents)}</li>)}</ul>;
    return <p key={idx} className="my-2.5 text-[var(--ink-2)]">{parseInlineStyles(block.content || '', citations, onSelectDocument, documents)}</p>;
  })}</div>;
};
...
export const ActionRouterCard: React.FC<{
  ...
  const handleShare = async () => { try { await navigator.clipboard.writeText(window.location.href); setShareCopied(true); setTimeout(() => setShareCopied(false), 2000); } catch {} };
  const exportAsPDF = async () => { try { setExporting('pdf'); onExportPDF(userPrompt?.trim() || 'Signal87 Export', msg.text || ''); } finally { setExporting(null); } };
  const exportAsWord = async () => { try { setExporting('word'); const source = document.querySelector(`[data-export-message-id="${msg.id}"]`) as HTMLElement | null; await exportResult({ element: source, format: 'word', title: userPrompt?.trim() || 'Signal87 Export', filename: 'Signal87-AI-Brief' }); } finally { setExporting(null); } };
  const exportAsExcel = async () => { try { setExporting('excel'); const source = document.querySelector(`[data-export-message-id="${msg.id}"]`) as HTMLElement | null; await exportResult({ element: source, format: 'excel', title: userPrompt?.trim() || 'Signal87 Export', filename: 'Signal87-AI-Brief' }); } finally { setExporting(null); } };
  const excelData = msg.excelExportData;
  return <div className="flex flex-wrap items-center gap-2 mt-3">
    <button onClick={() => onCopy(msg.id, msg.text || '')} className="px-3 py-1.5 border border-[var(--rule)] rounded-[3px] text-xs font-medium text-[var(--ink-2)] hover:text-[var(--ink)] hover:bg-[var(--surface-2)] transition-colors cursor-pointer">{copiedMsgId === msg.id ? 'Copied' : 'Copy'}</button>
    ...
    <button onClick={handleShare} className="px-3 py-1.5 border border-[var(--rule)] rounded-[3px] text-xs font-medium text-[var(--ink-2)] hover:text-[var(--ink)] hover:bg-[var(--surface-2)] transition-colors cursor-pointer flex items-center gap-1.5"><Share2 size={13} /> {shareCopied ? 'Copied' : 'Share'}</button>
