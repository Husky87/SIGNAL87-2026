...
import { checkoutPriceForPlan } from '../src/lib/billingPlans.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const priceId = checkoutPriceForPlan(req.body?.plan, {
    pro100: process.env.STRIPE_PRICE_ID_100,
    unlimited: process.env.STRIPE_PRICE_ID,
  });
  if (req.body?.plan !== 'pro_100' && req.body?.plan !== 'unlimited') return json(res, 400, { error: 'Choose a valid subscription plan.' });
