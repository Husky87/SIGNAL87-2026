...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try { await requireFirebaseUser(req.headers.authorization); } catch (error: any) { return json(res, 401, { error: 'Unauthorized', details: error?.message || 'Valid Firebase ID token required' }); }
  ...
  const params = new URLSearchParams({ mode: 'subscription', 'line_items[0][price]': priceId, 'line_items[0][quantity]': '1', customer_email: email, success_url: `${SITE_URL}/?billing=success`, cancel_url: `${SITE_URL}/?billing=cancelled`, 'subscription_data[metadata][signal87_email]': email });
  try {
